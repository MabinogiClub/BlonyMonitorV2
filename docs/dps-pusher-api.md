...
客户端识别到角色后会在后台查询排行状态；如果当前频道能确定服务器且新服务端明确返回 `serverId` 缺失或不同，客户端会使用查询到的原排行模式发起一次签名 `PUT` 自动补全服务器字段。面对不返回该字段的旧服务端，客户端保持旧流程，不会重复补发请求。
...
`serverId` 为当前游戏服务器的稳定 ID，由客户端根据当前频道自动填写：
...
当服务器未知或历史记录尚未补全时，`serverId` 返回 `null`。mblogs 列表会将这种记录显示为“未知”。
...
上传仍只有一个 `file`。`targets[]` 中不重复放置 `buffCoverage`，旧服务端可忽略未知的
`extensions` 根字段；新服务端从 `extensions.buffMonitor` 读取完整 Buff 数据。整个 gzip
文件统一参与 `contentSha256` 和 HMAC 签名，因此伤害数据与 Buff 数据使用同一份完整性校验。
...
| 1.5 | 2026-08-07 | 排行参与状态增加 `serverId`，mblogs 列表显示服务器 |
